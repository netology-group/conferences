...
use svc_agent::{Authenticable, mqtt::{IncomingRequestProperties, ResponseStatus}};
...
impl RequestHandler for ReadHandler {
    ...
    async fn handle<C: Context>(
        ...
        if reqp.as_account_id().label() != "dispatcher" || reqp.as_account_id().audience() != context.agent_id().as_account_id().audience() {
            return Err(anyhow!(
